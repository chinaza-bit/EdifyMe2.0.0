...
// Locally this points at your dev server. Uses 127.0.0.1 (not "localhost")
// because that's what Live Server serves the frontend from by default, and
// the login cookie only works when frontend + backend hostnames match exactly.
// After deploying, replace the production URL below with your real Render URL.
...
// On Render's free tier the server can be asleep, so a plain network failure
// here (not a normal 401) means "still waking up" — show that and keep
// retrying instead of silently dropping the user onto a login screen whose
// next request will just fail the same way.
async function pingUntilAwake() {
  const wakeMsg = document.getElementById('wake-message');
  let attempt = 0;
  while (true) {
    try {
      await fetch(API.replace('/api', '/api/health'), { credentials: 'include' });
      wakeMsg.classList.add('hidden');
      return;
    } catch (_) {
      attempt++;
      wakeMsg.classList.remove('hidden');
      if (attempt > 1) {
        wakeMsg.textContent = `Still waking up the server (attempt ${attempt})... this only happens after a few minutes of inactivity.`;
      }
      await new Promise(r => setTimeout(r, 4000));
      if (attempt >= 20) { // ~80s ceiling — something's genuinely wrong past this point
        wakeMsg.textContent = "This is taking longer than usual. Check your internet connection, or the server may be down.";
        return;
      }
    }
  }
